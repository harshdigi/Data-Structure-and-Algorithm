...
int solve(vector<int> nums){
    // tortoise method
        long long int slow = nums[0]; // it takes only one step ahead at a time
        long long int fast = nums[0]; // it takes two steps ahead at a time
        
        // iterate until slow and fast pointer colloids
        do{
            slow = nums[slow];
            fast = nums[nums[fast]];
        ...
        fast = nums[0]; // reassign fast pointer to nums[0] and now it will take only one step ahead at a time 
        // iterate until slow and fast colloid once again this time when they colloid, slow pointer will hold the answer which we require so simply return it.
        while(slow!=fast) {
            slow = nums[slow];
            fast = nums[fast];
        } 
